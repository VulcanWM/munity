...
from functions import (
    ...
    addmoney,
    addxp,
    changesonglyricscore,
    xpleaderboard,
    moneyleaderboard,
    changealbumcoverscore,
    getrandomalbumcover,
    getalbumnames,
    coverimagetobyte,
    getalbumcover,
...
def index():
    return render_template(
        "index.html", artist=getcookie("artist"), username=getcookie("User")
    )
...
def getartistfunc():
    if request.method != "POST":
        return render_template(
            "setartist.html", artist=getcookie("artist"), username=getcookie("User")
        )
    ...
    if searchartist(name) == False:
        return render_template(
            ...
            error=f"{name} is not a real artist in Spotify!",
        )
    username = getcookie("User")
    delcookies()
...
def loginfunc():
    if request.method == "POST":
        ...
        if getuser(username) == False:
            return render_template(
                "login.html",
                error="That is not a username!",
                artist=getcookie("artist"),
            )
        password = request.form["password"]
        if check_password_hash(gethashpass(username), password) == False:
            return render_template(
                "login.html", error="Wrong password!", artist=getcookie("artist")
            )
        addcookie("User", username)
    return redirect("/")
...
def userprofile(username):
    if getuser(username) == False:
        return redirect("/")
    if getcookie("User") != False and getcookie("User") == username:
...
def leaderboardpage():
    ...
    if getcookie("User") == False:
        return render_template(
            "leaderboard.html", xplb=xplb, moneylb=moneylb, logged=False
        )
    else:
        return render_template(
            "leaderboard.html",
...
def guessalbum():
    if request.method == "POST":
        ...
        if song.lower() == accsong.lower():
            ...
            addcookie("artist", name)
            if song.lower() in getalbumnames(name) and coverimagetobyte(
                getalbumcover(song, name)
            ) == coverimagetobyte(getalbumcover(accsong, name)):
                points = points + 1
                if songnumber > 9:
                    if username != False:
                        addxp(username, points)
